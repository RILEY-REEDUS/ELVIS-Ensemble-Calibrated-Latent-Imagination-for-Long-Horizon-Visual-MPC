...
class TensorWrapper(gym.Wrapper):
	...
	def __init__(self, env):
		super().__init__(env)
		self._wrapped_vectorized = env.__class__.__name__ == 'SB3Vectorized' # Use 'Vectorized' if not using SB3Vectorized
	...
	def step(self, action, **kwargs):
		...
		if isinstance(info, list):
			info = tuple(info) # This is for SB3Vectorized compatibility
