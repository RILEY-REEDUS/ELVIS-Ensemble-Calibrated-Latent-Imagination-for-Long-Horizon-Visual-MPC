...
class RunningScale(torch.nn.Module):
	...
	def __init__(self, cfg):
		...
		self.register_buffer('_percentiles', torch.tensor([5, 95], dtype=torch.float32, device=torch.device('cuda:0') \
                                    if torch.cuda.is_available() else torch.device('cpu')))
		self.register_buffer('offset', torch.zeros(1, dtype=torch.float32, device=torch.device('cuda:0') \
                                    if torch.cuda.is_available() else torch.device('cpu')))
	...
	def update(self, x):
		...
		offset = percentiles[0]
		self.offset.data.lerp_(offset, self.cfg.tau)
