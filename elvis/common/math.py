...
def symlog_mse(pred, target):
    """
    Compute the mean squared error after applying the symlog transform.
    ...
    Returns:
        Tensor: Scalar loss computed as the mean squared error between
                symlog-transformed predictions and targets.
    """
    pred_symlog = symlog(pred)
    target_symlog = symlog(target)
    loss = F.mse_loss(pred_symlog, target_symlog, reduction='mean')
    return loss
